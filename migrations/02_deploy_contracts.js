...
module.exports = async (deployer, network, [account]) => {
  ...
  {
    const ClipperMomRecoverOwnership = artifacts.require('ClipperMomRecoverOwnership');
    const tempSetOwner = await artifact_deploy(ClipperMomRecoverOwnership);
    const jsonInterface = {
      type: 'function',
      name: 'run',
      inputs: [
    ...
    const calldata = web3.eth.abi.encodeFunctionCall(jsonInterface, []);
    await proxyDeployer.methods['execute(address,bytes)'](tempSetOwner.address, calldata);
  }
